...
void Scene::rasterize(Triangle oTriangle) {
	...
	for (int i = 0; i < 3; i++) {
		xMax = max(t.coords[i][0], xMax);
		xMin = min(t.coords[i][0], xMin);
		yMax = max(t.coords[i][1], yMax);
		yMin = min(t.coords[i][1], yMin);
	}

	Vertex v0(t.coords[0][0], t.coords[0][1], 1, 1);
	Vertex v1(t.coords[1][0], t.coords[1][1], 1, 1);
	Vertex v2(t.coords[2][0], t.coords[2][1], 1, 1);
	...
	for (int i = xMin; i <= xMax && i < fb.width; i++) {
		for (int j = yMin; j <= yMax && j < fb.height; j++) {
			if ((e0 >= 0 &&
				...
				pix[0] = (u08)(oTriangle.c[0][0] * 255);
				pix[1] = (u08)(oTriangle.c[0][1] * 255);
				pix[2] = (u08)(oTriangle.c[0][2] * 255);
			}
			e0 += f0.y;
			e1 += f1.y;
			e2 += f2.y;
