...
class Texture {
	...
	public:
		...
		void getInterColor(float uf, float vf, u08 *color) {
			float u = (uf - floor(uf)) * width;
			float v = (vf - floor(vf)) * height;
			int uMin = max(min((int)floor(u), width - 1), 0);
			int uMax = max(min((int)ceil(u), width - 1), 0);
			int vMin = max(min((int)floor(v), height - 1), 0);
			int vMax = max(min((int)ceil(v), height - 1), 0);
